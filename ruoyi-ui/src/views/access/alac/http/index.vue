    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['access:appl:http:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
              ...
              v-hasPermi="['access:appl:http:edit']"
          >修改
          </el-button>
          <el-button
              size="mini"
              ...
              v-hasPermi="['access:appl:http:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
<script>
import {addHttp, delHttp, getHttp, listHttp, updateHttp} from "@/api/access/appl/http";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHttp(this.queryParams).then(response => {
        this.httpList = response.data;
        this.loading = false;
      });
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "新增HTTP访问控制";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      getHttp(row.id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改HTTP访问控制";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateHttp(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addHttp(this.form).then(response => {
